...
function SummaryTabContent({ data, onEdit }) {
  ...
  // Mirrors Summary.jsx: CIF/FOB and GST totals come from the saved item
  // rows (CIFFOB, GSTAmount) — not from the invoice table. Unsaved/pending
  // n8n items naturally contribute 0 here since they don't have these
  // fields yet.
  const sumItemValue = items.reduce(
    (sum, it) => sum + toNum(it.TotalLineAmount),
    0,
  );
  const totalItemCifValue = items.reduce(
    (sum, it) => sum + toNum(it.CIFFOB),
    0,
  );
  const totalItemGstAmount = items.reduce(
    (sum, it) => sum + toNum(it.GSTAmount),
    0,
  );

  // Invoice-level totals — CIF comes from each invoice's computed
  // costInsuranceFreight.amountSgd; GST is the shared gstAmountSgd field
  // invoiceTab.js stores flat on each saved invoice row (not nested under
  // inv.gst — that path never existed in the saved data).
  ...
  const totalInvoiceGstAmount = invoices.reduce(
    (sum, inv) => sum + toNum(inv?.gstAmountSgd),
    0,
  );

  const totalGstValue = totalItemGstAmount;
  ...
  // SUM OF ITEM AMOUNT — grouped by each item's own currency, summing
  // TotalLineAmount, exactly like Summary.jsx's groupedItems.
...
export default function DeclarationPanel({
  ...
  const mergeMultiDeclarationTotals = (baseData) => {
    const mergedItems = dataList.flatMap((d) =>
      Array.isArray(d.items) ? d.items : [],
    );
    const mergedInvoices = dataList.flatMap((d) =>
  ...
  const postHeader = async (d, ids) => {
    ...
    const totalGSTTaxAmt = items.reduce(
      (sum, it) => sum + toNum(it.GSTAmount),
      0,
    );

    const totalExDutyAmt = items.reduce(
      (sum, it) => sum + toNum(it.ExciseDutyAmount),
      0,
    );
    const totalCusDutyAmt = items.reduce(
      (sum, it) => sum + toNum(it.CustomsDutyAmount),
      0,
    );
    const totalODutyAmt = items.reduce(
      (sum, it) => sum + toNum(it.OtherTaxAmount),
      0,
    );
    const totalAmountPayable =
      d.DeclarationType === "DNG : Duty & GST"
    ...
    const payload = {
      ...
      TotalExDutyAmt: totalExDutyAmt,
      TotalCusDutyAmt: totalCusDutyAmt,
    ...
    try {
      await API.post("inpayment/postInHeaderTable/", {
        ...payload,
        ReleaseLocName: d.ReleaseLocation?.Name || "",
      });
    } catch (mirrorErr) {
  ...
  const handleSaveCurrent = async () => {
    ...
    const isSingleMerge = hasMultiple && declareMode === "single";
    ...
    try {
      ...
      alert(`Saved Permit: ${ids.PermitId}`);
      ...
      if (hasMultiple && declareMode === "separate") {
        // Only this one declaration is done — remove it from the working
        // set but keep the email (and any remaining declarations) open.
            ...
            <>
              ...
              {activeTab === "summary" && (
                <div className="decl-body-save">
                  ...
                  >
                    {busy ? "Saving Permit…" : "Save Permit"}
                  </button>
                </div>
              )}
            </>
